...
public final class UCEDefaultActivity extends Activity {
   ...
   // private Throwable throwable;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Throwable exception = (Throwable) extras.getSerializable("exception");
...
//                }
    ...
    public void getCrash(Throwable paramThrowable) {

        try {
            String URL = "http://appvaultnew.betaserver.us/api/crash-logs";
            JSONObject params = new JSONObject();
            params.put("row_guid", "fcjhgvjrenresdre7568kfndkredjvnkmhn45566gh");
            params.put("line_number", String.valueOf(paramThrowable.getCause().getStackTrace()[0].getLineNumber()));
            params.put("file_name", paramThrowable.getCause().getStackTrace()[0].getFileName());
            params.put("method_name", paramThrowable.getCause().getStackTrace()[0].getMethodName());
            params.put("class_name", paramThrowable.getCause().getStackTrace()[0].getClassName());
            params.put("phone_model", Build.MANUFACTURER + Build.MODEL);
            params.put("model_version", String.valueOf(Build.VERSION.SDK_INT));
            params.put("app_guid", "z0oLDvsmolfWYlMfQVS6t3NwWuxGfkM5");
            params.put("app_version", BuildConfig.VERSION_NAME);
            params.put("device_type", "android");
            //  final String mRequestBody = params.toString();

            Log.i("LOG_VOLLEYPARAMS", String.valueOf(params));
            final RequestQueue requestQueue = Volley.newRequestQueue(getApplicationContext());
            JsonObjectRequest stringRequest = new JsonObjectRequest(Request.Method.POST, URL, params,
                    new Response.Listener<JSONObject>() {
                        @Override
                        public void onResponse(JSONObject response) {
                            //if (response.getString("status") == response.getString("true"))
                            Log.i("LOG_VOLLEY", response.toString());
                            Toast.makeText(getApplicationContext(),"Succefully Sent",Toast.LENGTH_SHORT).show();
            ...
            Log.d("TAG","IS REQUEST ADDED "+requestQueue.add(stringRequest));
